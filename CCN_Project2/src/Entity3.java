public class Entity3 extends Entity {
	...
	int mincost[];
	public Entity3() {
		System.out.println("Constructor of Node 3 is called");
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 4; j++)
		...
		int[] distanceVector = { 7,999,2,0 };

	      for(int i = 0; i < distanceTable.length; i++){
	        if(i != 1 && i != 3){
	          Packet p = new Packet(3, i, distanceVector);
		...
		System.out.println("The update method is called by "+"source "+p.getSource()+" to destination "+p.getDest());
		 ...
		 boolean updated = false;
	      ...
	      if(updated){
	    	  ...
	    	  for(int i =0; i < distanceTable.length; i++){
	            if(i != 1 && i != 3){
	              Packet newPacket = new Packet(3, i, distanceVector);
