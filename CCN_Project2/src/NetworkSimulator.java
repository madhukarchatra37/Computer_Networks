...
public class NetworkSimulator {
	...
	public void runSimulator() {
		...
		while (true) {
			...
			if (traceLevel > 1) {
				System.out.println();
				System.out.println("main(): event received.  t=" + next.getTime() + ", node=" + next.getEntity());
	 ...
	 * possibly send (e.g. Entity 1 could send a packet from 0 to 3). This
	...
	public static void toLayer2(Packet p) {
		...
		if (p.getSource() == p.getDest()) {
			System.out.println("toLayer2(): WARNING: Identical source and " + "destination in packet; ignoring.");
			return;
		}
		if (cost[p.getSource()][p.getDest()] == 999) {
			System.out.println("toLayer2(): WARNING: Source and destination " + "not connected; ignoring.");
			return;
		}
